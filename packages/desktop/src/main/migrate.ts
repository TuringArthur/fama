...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.fama.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `fama.settings.dat` is special: it maps to the `fama.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"fama.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // fama.settings.dat → the electron settings store ("fama.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "fama.global.dat").
  const storeName = filename === "fama.settings.dat" ? "fama.settings" : filename
