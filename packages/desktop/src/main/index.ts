...
const TEST_ONBOARDING = process.env.FAMA_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  const onboardingTestRoot = ((): string | undefined => {
    ...
    const root = join(tmpdir(), `fama-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Fama Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("fama://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("fama")
  registerRendererProtocol()
  setDockIcon()
