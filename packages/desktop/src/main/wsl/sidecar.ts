...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export FAMA_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export FAMA_CLIENT=desktop",
    `export FAMA_SERVER_USERNAME=${shellEscape(username)}`,
    `export FAMA_SERVER_PASSWORD=${shellEscape(password)}`,
