  ...
  privacy: Schema.optional(
    Schema.Struct({
      offline: Schema.optional(Schema.Boolean).annotate({
        description:
          "Enable offline mode: hides network-facing tools (webfetch/websearch), skips online law/case lookup and remote MCP servers, and disables auto-update. Intended for confidential legal workloads.",
      }),
    }),
  ).annotate({ description: "Privacy settings (Fama). See docs/security.md" }),
