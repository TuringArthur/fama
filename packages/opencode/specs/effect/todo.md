...
- HTTP error middleware still guesses status codes from error names â€?  some entries (e.g. storage `NotFound`, provider auth) can now be
...
- `Global.make()` still reads mutable `Flag.fama_CONFIG_DIR`.
