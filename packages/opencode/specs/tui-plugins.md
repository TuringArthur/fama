...
- If `engines.fama` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.fama` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.fama`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.fama`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.fama` dir or `FAMA_CONFIG_DIR`, OpenCode installs `@opencode-ai/plugin` into that dir and writes:
...
- Local plugins persist installed themes under the local `.fama/themes` area near the plugin config source.
...
- `--pure` / `FAMA_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.fama/plugins/tui-smoke.tsx`
- Local vim plugin: `.fama/plugins/tui-vim.tsx`
- Local smoke config: `.fama/tui.json`
- Local smoke theme: `.fama/plugins/smoke-theme.json`
