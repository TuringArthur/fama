...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@fama-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@fama-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@fama-ai/tui/context/theme"
import { Global } from "@fama-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@fama-ai/core/util/flock"
import { Flag } from "@fama-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@fama-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@fama-ai/core/v1/config/plugin"
import { createCommandShim } from "@fama-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@fama-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".fama"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".fama", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".fama", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.FAMA_PURE ? [] : pluginOrigins
    if (Flag.FAMA_PURE && pluginOrigins.length) {
    }
