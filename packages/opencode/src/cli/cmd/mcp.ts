...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .fama/ subdirectory too)
  const candidates = [path.join(baseDir, "fama.json"), path.join(baseDir, "fama.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".fama", "fama.json"), path.join(baseDir, ".fama", "fama.jsonc"))
  }
