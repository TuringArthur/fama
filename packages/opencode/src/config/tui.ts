...
import { isRecord } from "@fama-ai/tui/util/record"
...
import { TuiKeybind } from "@fama-ai/tui/config/keybind"
...
import { TuiConfig } from "@fama-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@fama/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and FAMA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.FAMA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit FAMA_TUI_CONFIG override, if set.
  if (Flag.FAMA_TUI_CONFIG) {
    const configFile = Flag.FAMA_TUI_CONFIG
  ...
  // 4. `.fama` directories (and FAMA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".fama") || dir === Flag.FAMA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".fama") && dir !== Flag.FAMA_CONFIG_DIR) continue
