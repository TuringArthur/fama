...
`,
  ...
  const pure = process.env.FAMA_PURE
  const meta = process.env.FAMA_PLUGIN_META_FILE
  process.env.FAMA_PURE = "1"
  process.env.FAMA_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.FAMA_PURE
    } else {
      process.env.FAMA_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.FAMA_PLUGIN_META_FILE
    } else {
      process.env.FAMA_PLUGIN_META_FILE = meta
    }
  }
})
