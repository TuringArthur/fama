...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.FAMA_CONFIG
  delete process.env.FAMA_TUI_CONFIG
...
it.instance("project config takes precedence over FAMA_TUI_CONFIG (matches FAMA_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("FAMA_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from FAMA_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
